...
* @brief D* Lite 알고리즘 기반 3-에이전트 자동 주차 시뮬레이션 (연결 리스트 큐 및 개선된 작업 처리 로직 적용, 작업 딜레이 추가)
*/
...
#define PARKING_TIME 5               // [ADDED] 주차 작업에 소요되는 시간 (시뮬레이션 스텝 기준)
#define COLLECTION_TIME 5            // [ADDED] 차량 수거 작업에 소요되는 시간 (시뮬레이션 스텝 기준)
 ...
 * D* Lite 알고리즘에서 사용되며, k1과 k2 두 값으로 우선순위를 비교합니다.
 */
...
typedef enum {
    ...
    PERFORMING_PARKING,         // [ADDED] 주차 작업 수행 중
    RETURNING_HOME_EMPTY,       // 주차 후 빈 몸으로 기지로 복귀하는 중
    GOING_TO_COLLECT,           // 출차할 차를 가지러 가는 중
    PERFORMING_COLLECTION,      // [ADDED] 차량 수거 작업 수행 중
...
typedef struct {
    ...
    int action_timer;           // [ADDED] 주차/수거 작업 잔여 시간
} Agent;

/** @struct PriorityQueue
 * @brief 최소 힙(min-heap)으로 구현된 우선순위 큐.
 * D* Lite 알고리즘에서 비용이 가장 낮은 노드를 효율적으로 찾기 위해 사용됩니다.
 */
typedef struct {
...
/** @struct Pathfinder
* @brief D* Lite 경로 탐색 중의 상태(PQ, 시작/목표)를 임시로 관리.
*/
typedef struct {
    PriorityQueue pq;           // 경로 탐색을 위한 우선순위 큐
    Node* start_node;           // 탐색 시작 노드 (에이전트의 현재 위치)
...
void pathfinder_compute_shortest_path(Pathfinder* pf, GridMap* map, AgentManager* agent_manager); // D* Lite 알고리즘으로 최단 경로 계산
...
void agent_manager_update_action_state(AgentManager* manager, Logger* logger); // [ADDED] 주차/수거 작업 중인 에이전트 상태 업데이트
...
static void simulation_display_status(const Simulation* sim) {
    ...
    // --- 3. 에이전트 상태 정보 버퍼에 쓰기 ---
    // [MODIFIED] 상태 문자열 및 색상 배열에 새로운 상태 추가
    const char* agent_state_strings[] = { "대기", "주차 공간으로 이동", "주차 작업 중", "기지 복귀(빈 차)", "수거하러 이동", "수거 작업 중", "출차 중(차량 탑승)", "충전소로 이동", "충전 중", "기지 복귀(충전 후)" };
    const char* agent_state_colors[] = { C_GRY, C_YEL, C_B_YEL, C_CYN, C_YEL, C_B_YEL, C_GRN, C_B_RED, C_RED, C_CYN };
...
int grid_is_node_blocked(const GridMap* map, const AgentManager* agent_manager, const Node* n) {
    ...
    for (int i = 0; i < MAX_AGENTS; i++) { // 모든 에이전트에 대해
        // 다른 에이전트가 해당 위치에서 충전, 주차, 수거 작업을 하고 있다면
...
/**
 * @brief D* Lite 알고리즘에 따라 노드의 우선순위 키를 계산합니다.
 * @param pf 경로 탐색기. start_node는 에이전트의 현재 위치.
 * @param n 키를 계산할 노드.
 * @return 계산된 Key 구조체.
 */
static Key calculate_key(const Pathfinder* pf, const Node* n) {
    // k1 = min(g(n), rhs(n)) + h(start, n), k2 = min(g(n), rhs(n))
 ...
 * D* Lite에서는 노드 u의 rhs 값은 그 이웃(successor)들의 g값에 기반하여 업데이트됩니다.
 */
static void path_update_vertex(Pathfinder* pf, GridMap* map, const AgentManager* agent_manager, Node* u) {
    if (u != pf->goal_node) { // 목표 노드가 아니라면
        ...
        // u의 이웃(successor)들을 순회하며 rhs(u) = min(c(u, s') + g(s'))를 계산
        for (int i = 0; i < NUM_DIRECTIONS; i++) {
            ...
            if (grid_is_valid_coord(nx, ny)) { // 이웃이 그리드 내에 있다면
                ...
                // 이웃으로 가는 비용은 1.0으로 고정. D* Lite는 c(u, s')를 사용.
                // 여기서는 u와 successor 사이의 비용이 1.0이라고 가정.
...
Pathfinder* pathfinder_create(Node* start, Node* goal) {
    ...
    pf->start_node = start; // 시작 노드 설정 (에이전트 현재 위치)
...
/**
 * @brief D* Lite 알고리즘의 메인 로직을 수행하여 최단 경로를 계산합니다.
 */
...
AgentManager* agent_manager_create() {
    ...
    for (int i = 0; i < MAX_AGENTS; i++) { // 각 에이전트 초기화
        ...
        manager->agents[i].action_timer = 0; // [ADDED] action_timer 초기화
    }
    return manager; // 생성된 manager 객체 포인터 반환
}
...
static void agent_set_goal(Agent* agent, GridMap* map, AgentManager* agent_manager, Logger* logger) {
    ...
    // 대기 중, 충전 중, 작업 중이거나 이미 목표가 설정된 경우엔 새로운 목표를 설정하지 않음
    if (agent->state == IDLE || agent->state == CHARGING || agent->state == PERFORMING_PARKING || agent->state == PERFORMING_COLLECTION || agent->goal != NULL) return;
...
void agent_manager_plan_and_resolve_collisions(AgentManager* manager, GridMap* map, Logger* logger, Node* next_pos[MAX_AGENTS]) {
    ...
    for (int i = 0; i < MAX_AGENTS; i++) {
        ...
        if (agent->goal == NULL && agent->state != IDLE && agent->state != CHARGING && agent->state != PERFORMING_PARKING && agent->state != PERFORMING_COLLECTION) {
    ...
    for (int i = 0; i < MAX_AGENTS; i++) {
        Agent* agent = &manager->agents[i];
        if (agent->state == IDLE || agent->state == CHARGING || agent->state == PERFORMING_PARKING || agent->state == PERFORMING_COLLECTION || agent->goal == NULL) {
            continue; // 비활성 상태이거나 목표가 없는 에이전트는 건너뜁니다.
        }

        // 이 에이전트의 경로 탐색을 위해 다른 에이전트들을 임시 장애물로 설정
        // D* Lite의 동적 재계획 특성을 활용하는 부분입니다.
...
void agent_manager_update_state_after_move(AgentManager* manager, ScenarioManager* scenario, Logger* logger) {
    for (int i = 0; i < MAX_AGENTS; i++) {
        ...
        if (agent->state == IDLE || agent->state == CHARGING || agent->state == PERFORMING_PARKING || agent->state == PERFORMING_COLLECTION || !agent->goal || agent->pos != agent->goal) {
            continue;
        }

        Node* reached_goal = agent->goal; // 도달한 목표 노드

        // 충전소와 기지는 계속 사용해야 하므로 예약을 즉시 해제하지 않음
        if (agent->state != GOING_TO_CHARGE && agent->state != RETURNING_HOME_EMPTY && agent->state != RETURNING_WITH_CAR && agent->state != RETURNING_HOME_MAINTENANCE) {
            reached_goal->reserved_by_agent = -1;
        }
        agent->goal = NULL; // 에이전트의 목표 초기화

        switch (agent->state) {
        case GOING_TO_PARK: // [MODIFIED] 주차하러 가는 중이었다면
            reached_goal->is_parked = TRUE; // 주차 완료 처리
        ...
        case RETURNING_HOME_EMPTY: // 주차 후 기지로 복귀 중이었다면
            ...
            agent->state = IDLE;
            reached_goal->reserved_by_agent = -1; // 기지 예약 해제
            break;

        case GOING_TO_COLLECT: // [MODIFIED] 수거하러 가는 중이었다면
            reached_goal->is_parked = FALSE; // 차량 수거 처리
            manager->total_cars_parked--;
...
/**
 * @brief [ADDED] 주차 또는 수거 작업 중인 에이전트의 상태를 업데이트합니다.
 */
void agent_manager_update_action_state(AgentManager* manager, Logger* logger) {
    for (int i = 0; i < MAX_AGENTS; i++) {
        Agent* agent = &manager->agents[i];
        if (agent->state == PERFORMING_PARKING) { // 주차 작업 중인 에이전트
            agent->action_timer--;
            if (agent->action_timer <= 0) {
                ...
                agent->goal = NULL; // 목표를 초기화하여 기지로 돌아가는 새 목표를 설정하도록 함
            }
        }
        else if (agent->state == PERFORMING_COLLECTION) { // 수거 작업 중인 에이전트
            agent->action_timer--;
            if (agent->action_timer <= 0) {
                logger_log(logger, "[%sExit%s] Agent %c, 수거 작업 완료. 출차 지점으로 이동합니다.", C_GRN, C_NRM, agent->symbol);
                agent->state = RETURNING_WITH_CAR;
                agent->goal = NULL; // 목표를 초기화하여 기지(출차 지점)로 돌아가는 새 목표를 설정하도록 함
            }
        }
    }
}
...
void simulation_run(Simulation* sim) {
    while (TRUE) { // 무한 루프
        ...
        agent_manager_update_action_state(sim->agent_manager, sim->logger); // 2. [ADDED] 주차/수거 작업 상태 업데이트
        simulation_update_state(sim); // 3. 작업 할당 등 전체 상태 업데이트

        Node* next_pos[MAX_AGENTS]; // 각 에이전트의 다음 위치를 저장할 배열
        agent_manager_plan_and_resolve_collisions(sim->agent_manager, sim->map, sim->logger, next_pos); // 4. 경로 계획 및 충돌 해결

        // 계산된 다음 위치로 에이전트 이동
        for (int i = 0; i < MAX_AGENTS; i++) {
            // 타이머가 동작 중인 상태가 아닐 때만 이동
            AgentState currentState = sim->agent_manager->agents[i].state;
            if (currentState != CHARGING && currentState != PERFORMING_PARKING && currentState != PERFORMING_COLLECTION && next_pos[i] != NULL) {
        ...
        agent_manager_update_state_after_move(sim->agent_manager, sim->scenario_manager, sim->logger); // 5. 이동 후 목표 도달 등 상태 업데이트

        // 최적화된 렌더링 함수 호출
        simulation_display_status(sim); // 6. 현재 상태를 화면에 표시

        if (simulation_is_complete(sim)) break; // 7. 시뮬레이션 완료 조건 확인 후 루프 탈출
