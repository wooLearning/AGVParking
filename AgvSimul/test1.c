/**
* @file lpa_parking.c
* @brief LPA* 알고리즘 기반 3-에이전트 자동 주차 시뮬레이션 (상세 주석 추가 및 버그 수정 버전)
*/
...
#define GRID_HEIGHT 9           // 그리드 맵의 높이
...
/** @struct Task
 * @brief 작업 큐에 저장될 단일 작업을 나타내는 구조체.
 */
typedef struct { TaskType type; } Task; // 작업의 유형만 저장
...
typedef struct {
    ...
    Task task_queue[MAX_TASKS];         // 실시간 모드의 작업 큐 (원형 큐)
    int task_queue_head;                // 작업 큐의 시작 인덱스
    int task_queue_tail;                // 작업 큐의 끝 인덱스
...
// =============================================================================
// --- 6. 모듈별 구현 (GridMap, AgentManager, Logger, etc.) ---
// =============================================================================
...
/**
 * @brief 에이전트의 다음 이동을 계획합니다. (목표 설정 및 경로 계산)
 */
static void agent_plan_move(Agent* agent, GridMap* map, AgentManager* agent_manager, Logger* logger, Node** next_pos_out) {
    agent_set_goal(agent, map, agent_manager, logger); // 목표 설정

    if (agent->goal) { // 목표가 있다면
        // 출차할 차를 가지러 가는 경우, 목표 지점(주차된 차)을 일시적으로 길로 취급
        int goal_was_parked = (agent->state == GOING_TO_COLLECT && agent->goal->is_parked);
        if (goal_was_parked) agent->goal->is_parked = FALSE; // 임시로 '주차 안됨' 상태로 변경

        Pathfinder* pf = pathfinder_create(agent->pos, agent->goal); // 경로 탐색기 생성
        pathfinder_compute_shortest_path(pf, map, agent_manager); // 경로 계산
        *next_pos_out = pathfinder_get_next_step(pf, map, agent_manager, agent->pos); // 다음 스텝 가져오기
        pathfinder_destroy(pf); // 경로 탐색기 메모리 해제

        if (goal_was_parked) agent->goal->is_parked = TRUE; // 원래 상태로 복원
    }
}
...
void agent_manager_plan_and_resolve_collisions(AgentManager* manager, GridMap* map, Logger* logger, Node* next_pos[MAX_AGENTS]) {
    ...
    for (int i = 0; i < MAX_AGENTS; i++) {
        ...
        if (agent->state == IDLE || agent->state == CHARGING) continue; // 대기 또는 충전 중이면 건너뜀

        // 다른 에이전트들을 임시 장애물로 설정하여 경로 계획
        Node* obstacles_to_clear[MAX_AGENTS];
        int obs_count = 0;
        for (int j = 0; j < MAX_AGENTS; j++) {
            if (i == j) continue; // 자기 자신은 제외
            // i보다 우선순위가 높은(먼저 계산된) 에이전트는 이미 계산된 다음 위치를, 아닌 경우는 현재 위치를 장애물로 설정
            Node* obs_node = (j < i) ? next_pos[j] : manager->agents[j].pos;
            if (obs_node) {
                obs_node->is_temp = TRUE; // 임시 장애물로 표시
                obstacles_to_clear[obs_count++] = obs_node; // 나중에 해제하기 위해 저장
            }
        }

        agent_plan_move(agent, map, manager, logger, &next_pos[i]); // 경로 계획
    ...
    for (int i = 0; i < MAX_AGENTS; i++) {
        for (int j = i + 1; j < MAX_AGENTS; j++) {
            if (manager->agents[i].state == IDLE || manager->agents[j].state == IDLE ||
                manager->agents[i].state == CHARGING || manager->agents[j].state == CHARGING) continue; // 비활성 에이전트는 무시
...
/**
 * @brief 작업 큐에 새로운 작업을 추가합니다. (실시간 모드용)
 */
static void add_task_to_queue(ScenarioManager* scenario, TaskType type) {
    ...
    scenario->task_queue[scenario->task_queue_tail] = (Task){ type }; // 큐의 끝(tail)에 작업 추가
    scenario->task_queue_tail = (scenario->task_queue_tail + 1) % MAX_TASKS; // tail 인덱스를 다음 위치로 이동 (원형 큐)
    scenario->task_count++; // 큐의 작업 수 증가
}

/**
 * @brief 작업 큐의 특정 위치에 있는 작업을 제거합니다.
 */
static void remove_task_from_queue_at(ScenarioManager* scenario, int relative_index) {
    if (relative_index < 0 || relative_index >= scenario->task_count) return; // 유효하지 않은 인덱스면 종료

    // 제거할 위치부터 끝까지 모든 요소를 한 칸씩 앞으로 당김
    for (int i = relative_index; i < scenario->task_count - 1; i++) {
        int current_idx = (scenario->task_queue_head + i) % MAX_TASKS;
        int next_idx = (scenario->task_queue_head + i + 1) % MAX_TASKS;
        scenario->task_queue[current_idx] = scenario->task_queue[next_idx];
    }
    // tail을 한 칸 뒤로 이동시켜 큐의 논리적 크기를 줄임
    scenario->task_queue_tail = (scenario->task_queue_tail - 1 + MAX_TASKS) % MAX_TASKS;
    scenario->task_count--; // 작업 수 감소
}
...
static void simulation_update_state(Simulation* sim) {
    ...
    for (int i = 0; i < MAX_AGENTS; i++) {
        ...
        if (agent->state == IDLE) { // 대기 중인 에이전트에게만 작업 할당 시도
            ...
            // [실시간 모드 작업 할당]
            else if (scenario->mode == MODE_REALTIME && scenario->task_count > 0) { // 큐에 작업이 있다면
                for (int j = 0; j < scenario->task_count; j++) { // 큐를 순회하며
                    int task_idx = (scenario->task_queue_head + j) % MAX_TASKS;
                    Task task = scenario->task_queue[task_idx];
                    if (task.type == TASK_PARK && agent_manager->total_cars_parked < map->num_goals) { // 주차 작업이고 공간이 있다면
                        agent->state = GOING_TO_PARK; // 주차 작업 할당
                        logger_log(logger, "[%sTASK%s] Agent %c, 주차 작업 할당.", C_CYN, C_NRM, agent->symbol);
                        remove_task_from_queue_at(scenario, j); // 큐에서 작업 제거
                        break; // 이 에이전트는 작업 할당 완료, 다음 에이전트로
                    }
                    else if (task.type == TASK_EXIT && agent_manager->total_cars_parked > 0) { // 출차 작업이고 차가 있다면
                        agent->state = GOING_TO_COLLECT; // 출차 작업 할당
                        logger_log(logger, "[%sTASK%s] Agent %c, 출차 작업 할당.", C_CYN, C_NRM, agent->symbol);
                        remove_task_from_queue_at(scenario, j); // 큐에서 작업 제거
                        break; // 이 에이전트는 작업 할당 완료
                    }
                }
            }
        }
